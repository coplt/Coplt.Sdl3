...
if not <| Directory.Exists "./Coplt.Sdl3/Flags" then
    ignore <| Directory.CreateDirectory("./Coplt.Sdl3/Flags")

let ignores = Set [ "MouseButton" ]

let type_map = Map [ "Uint64", "ulong"; "Uint32", "uint"; "Uint8", "byte" ]
...
let flags_regex =
    Regex("typedef (?<type>Uint64|Uint32|Uint8) SDL_(?<name>[\w]+)Flags;")
...
for path in File.ReadLines("./HeadersToGen.txt") do
    ...
    if flags.Count > 0 then
        ...
        File.WriteAllText($"./Coplt.Sdl3/Flags/{file_name}.cs", code)
